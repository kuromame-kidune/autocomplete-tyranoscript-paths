...
export default class PathsProvider extends EventEmitter {
   ...
   */
  reloadScopes () {
    this._scopes = atom.config.get('autocomplete-tyranoscript-paths.scopes').slice(0) || []

    if (!atom.config.get('autocomplete-tyranoscript-paths.ignoreBuiltinScopes')) {
      this._scopes = this._scopes.concat(DefaultScopes)
    }

    for (var key in OptionScopes) {
      if (atom.config.get(`autocomplete-tyranoscript-paths.${key}`)) {
  ...
  _getSuggestionsForScope (scope, request, match) {
    ...
    if (extensions) {
      ...
      const regex = new RegExp(`(data).*(bgimage|bgm|fgimage|image|others|scenario|system|video).*(${extensions.join('|')})$`)
      files = files.filter(path => regex.test(path))
    }
    // [edit.end] ================================
    ...
    let suggestions = files.map(pathName => {
      const normalizeSlashes = atom.config.get('autocomplete-tyranoscript-paths.normalizeSlashes')
      ...
      // [edit.start] ==============================
      let ksFilePath = pathName
      let dataFolder = ['bgimage', 'bgm', 'fgimage', 'image', 'others', 'scenario', 'sound', 'system', 'video']
      let i = dataFolder.length
      let strLength = 0
      while (i--) {
          let pathIndex = pathName.indexOf('/'+dataFolder[i]+'/')
          if (pathIndex !== -1) {
              strLength = dataFolder[i].length
              ksFilePath = pathName.slice(pathIndex+strLength+2)
              break
          }
      }
      displayText = displayText.replace('data/', '')
  ...
  get suggestionPriority () {
    return atom.config.get('autocomplete-tyranoscript-paths.suggestionPriority')
  }
