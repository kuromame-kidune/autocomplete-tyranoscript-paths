  ...
  _isPathIgnored (path) {
    ...
    if (atom.config.get('core.excludeVcsIgnoredPaths')) {
      this._repositories.forEach(repository => {
        if (ignored) return
        const ignoreSubmodules = atom.config.get('autocomplete-tyranoscript-paths.ignoreSubmodules')
    ...
    if (atom.config.get('autocomplete-tyranoscript-paths.ignoredNames')) {
    ...
    const ignoredPatterns = atom.config.get('autocomplete-tyranoscript-paths.ignoredPatterns')
    if (ignoredPatterns) {
  ...
  _cacheDirectoryFilePaths (projectDirectory, directory) {
    ...
    return this._getDirectoryEntries(directory)
      .then(entries => {
        ...
        const maxFileCount = atom.config.get('autocomplete-tyranoscript-paths.maxFileCount')
        if (newPathsCount > maxFileCount && !this._cancelled) {
          atom.notifications.addError('autocomplete-tyranoscript-paths', {
            description: `Maximum file count of ${maxFileCount} has been exceeded. Path autocompletion will not work in this project.<br /><br /><a href="https://github.com/atom-community/autocomplete-tyranoscript-paths/wiki/Troubleshooting#maximum-file-limit-exceeded">Click here to learn more.</a>`,
            dismissable: true
          })
